...
public class AnalyticsService {
    ...
    public DashboardResponse dashboard(Long userId) {
        ...
        List<Budget> budgets = budgetRepository.findByUserIdOrderByCategoryNameAsc(userId);
        ...
        List<TransactionView> recent = transactionRepository.findByUserIdOrderByOccurredAtDesc(userId)
                .stream().limit(6).map(ViewMapper::transaction).toList();
    ...
    public MonthlyReportResponse monthlyReport(Long userId, Integer year, Integer month) {
        ...
        List<ExpenseTransaction> previousTransactions = transactionsForMonth(userId, target.minusMonths(1));
        List<Budget> budgets = budgetRepository.findByUserIdOrderByCategoryNameAsc(userId);

        BigDecimal currentTotal = total(currentTransactions);
        BigDecimal previousTotal = total(previousTransactions);
        double change = previousTotal.signum() == 0
        ...
        CategorySpend highest = spending.isEmpty()
        ...
        if (change > 0) insights.add("Spending increased by " + round(change) + "% compared with the previous month.");
        else if (change < 0) insights.add("Spending decreased by " + round(Math.abs(change)) + "% compared with the previous month.");
        else insights.add("Spending is unchanged compared with the previous month.");
        if (!spending.isEmpty()) insights.add(highest.category() + " was the highest category at ₹" + highest.amount().setScale(0, RoundingMode.HALF_UP) + ".");
        progress.stream().filter(BudgetProgress::warning).findFirst().ifPresent(p ->
                insights.add(p.category() + " has reached " + round(p.usedPercent()) + "% of its monthly limit."));
        if (progress.stream().noneMatch(BudgetProgress::warning)) insights.add("All configured categories are currently below their warning levels.");

        return new MonthlyReportResponse(
                target.getYear(), target.getMonthValue(), currentTotal, previousTotal,
                round(change), highest.category(), highest.amount(), spending, progress, insights);
    }
    ...
    public List<BudgetProgress> getBudgetProgress(Long userId) {
        ...
        return budgetProgress(
                budgetRepository.findByUserIdOrderByCategoryNameAsc(userId),
                transactionsForMonth(userId, current), current);
    }

    public List<ExpenseTransaction> transactionsForMonth(Long userId, YearMonth month) {
        LocalDateTime start = month.atDay(1).atStartOfDay();
        LocalDateTime end = month.plusMonths(1).atDay(1).atStartOfDay();
        return transactionRepository.findByUserIdAndOccurredAtBetweenOrderByOccurredAtDesc(userId, start, end);
    }
    ...
    public List<CategorySpend> categorySpending(List<ExpenseTransaction> transactions) {
        ...
        return grouped.entrySet().stream()
                .map(entry -> new CategorySpend(
                        entry.getKey().getName(), entry.getKey().getIcon(), entry.getValue(),
                        total.signum() == 0 ? 0 : round(entry.getValue()
    ...
    private List<BudgetProgress> budgetProgress(List<Budget> budgets,
        ...
        return budgets.stream().map(budget -> {
            ...
            double usedPercent = budget.getMonthlyLimit().signum() == 0 ? 0 : spent
                    .divide(budget.getMonthlyLimit(), 4, RoundingMode.HALF_UP)
                    .multiply(BigDecimal.valueOf(100)).doubleValue();
            BigDecimal projected = elapsedDays == 0 ? spent : spent
            ...
            return new BudgetProgress(
                    ...
                    budget.getCategory().getIcon(), spent, budget.getMonthlyLimit(),
                    budget.getWarningPercent(), round(usedPercent), projected,
                    usedPercent >= budget.getWarningPercent());
        }).toList();
    }

    public BigDecimal total(List<ExpenseTransaction> transactions) {
        return transactions.stream().map(ExpenseTransaction::getAmount)
                .reduce(BigDecimal.ZERO, BigDecimal::add);
    }
