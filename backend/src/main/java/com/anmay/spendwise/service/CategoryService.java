...
public class CategoryService {
  ...
  public List<CategoryView> list(Long userId) {
    return categoryRepository.findByUserIdOrderByNameAsc(userId).stream().map(this::view).toList();
  }
  ...
  public CategoryView create(CreateCategoryRequest request) {
    ...
    categoryRepository
        .findByUserIdAndNameIgnoreCase(request.userId(), request.name())
        .ifPresent(
            existing -> {
              throw new IllegalArgumentException("Category already exists");
            });
    String icon =
        request.icon() == null || request.icon().isBlank() ? "🏷️" : request.icon().trim();
    Category category =
        categoryRepository.save(new Category(user, request.name().trim(), icon, false));
    return view(category);
  }

  private CategoryView view(Category category) {
    return new CategoryView(
        category.getId(), category.getName(), category.getIcon(), category.isSystemDefined());
  }
}
