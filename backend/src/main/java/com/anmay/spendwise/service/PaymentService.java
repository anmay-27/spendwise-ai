...
public class PaymentService {
  ...
  private com.anmay.spendwise.finance.TransactionCrudService crud;
  ...
  public PaymentResponse pay(PaymentRequest request) {
    ...
    MlServiceClient.Prediction prediction =
            ...
            .orElseGet(
                () ->
                    categoryRepository
                        ...
                        .orElseThrow(
                            () -> new IllegalArgumentException("Other category is missing")));
    ...
    ExpenseTransaction transaction =
        new ExpenseTransaction(
            ...
            LocalDateTime.now());
    transaction.markWalletPayment();
    transactionRepository.saveAndFlush(transaction);
    crud.publish("TRANSACTION_CREATED", transaction);
  ...
  private String budgetWarning(Long userId, Category category) {
    return budgetRepository
        .findByUserIdAndCategoryIdAndMonth(userId, category.getId(), YearMonth.now().toString())
        .map(
            budget -> {
              BigDecimal spent =
                  analyticsService.total(
