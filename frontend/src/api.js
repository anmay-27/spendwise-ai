...
const GOOGLE_LOGIN_ENABLED =
  String(import.meta.env.VITE_GOOGLE_LOGIN_ENABLED || "false") === "true";
...
async function csrfToken() {
  // Spring may rotate the cookie after authentication. Read its current value
  // for every mutation rather than retaining a token from an earlier request.
  const currentToken = readCookie("XSRF-TOKEN");
  if (currentToken)
    return { token: currentToken, headerName: csrf?.headerName || "X-XSRF-TOKEN" };
  ...
  if (!rawToken) {
    throw new Error(
      "The CSRF security cookie was not created. Refresh the page and try again.",
    );
  }
...
export async function request(path, options = {}, retried = false) {
  ...
  if (
    ...
    if (!refreshing)
      refreshing = request("/auth/refresh", { method: "POST" }, true).finally(
        () => {
          refreshing = null;
        },
      );
...
export const api = {
  ...
  budgets: (month) => request("/budgets" + (month ? "?month=" + month : "")),
  setBudget: (payload) =>
    request("/budgets", {
      method: "POST",
      body: JSON.stringify(payload),
